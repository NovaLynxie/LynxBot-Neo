...
function createGuildEventLog(event, guild, ...args) {
    ...
    const embed = guildLogBaseEmbed(); // generate guild logs base embed and stores for reference calls
    let channel, member; // common placeholder variables used by all log events.
    switch (event) {
        // Channel Events
