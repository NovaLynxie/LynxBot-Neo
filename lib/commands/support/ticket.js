...
const { SlashCommandBuilder, InteractionContextType, ThreadAutoArchiveDuration, ChannelType, PermissionFlagsBits, EmbedBuilder, inlineCode } = require("discord.js");
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const ticketsModRole = serverSettings.get("ticketsModRole") ?? serverSettings.get("staffDefaultRole");
        ...
        async function updateTicketData(thread) {
            try {
                await Tickets.update({
                        ticketState: options.getString("reason")
                    },
        ...
        async function transcribeTicket(thread) {
            ...
            try {
                const threadOwner = client.users.resolve(thread.ownerId);
                await threadOwner.send({
                    content: "Here is your ticket transcript.",
                    files: [threadTranscript]
                });
            } catch (err) {
                logger.error(`Failed to send DM to ${threadOwner.username}! Maybe user has disabled their DMs for ${guild.name}?`);
                logger.error(`${err.name}: ${err.message}`);
                logger.debug(err.stack);
            };
        };
        ...
        if (subcommand === "close") {
            if (interaction.channel.isThreadOnly()) {
                ...
                return interaction.editReply({
                    content: `You do not have permission to use this command outside threads! Please run ${inlineCode("/ticket close")} in your ticket thread.`
                });
            };
        };
        if (subcommand === "lock") {
