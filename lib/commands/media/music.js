            ...
            case "play":
                ...
                if (subcommand === "file") {
                    ...
                    resource = createAudioResource(attachment.url, {
                        metadata: { url: options.getString("url"), type: schema.types.FILE, ...metadata } ?? null, inlineVolume: true
                    });
                ...
                if (subcommand === "link") {
                    ...
                    const linkStream = await fetchWebStream(options.getString("url"));
                    const metadata = await parseStream(linkStream);
                    resource = createAudioResource(options.getString("url"), {
                        metadata: { url: options.getString("url"), type: schema.types.STREAM, ...metadata } ?? null,
                        inlineVolume: true
                    });
                ...
                if (resource.metadata) {
                    if (resource.metadata.common) {
                        await interaction.editReply({
                            content: `Playing ${bold(`${resource.metadata.common.title} by ${resource.metadata.common.artist ?? "Unknown"}`)}!`
                        });
                        return voiceChannel.send({
                            content: `Playing ${bold(`${resource.metadata.common.title} by ${resource.metadata.common.artist}`)}! \nRequested by ${member}`
                        });
                    } else {
                        await interaction.editReply({
                    ...
                    await interaction.editReply({
                        content: "No metadata found! Playing audio file anyways."
                    });
                    return voiceChannel.send({ content: `Playing audio file uploaded by ${member}` });
                };
            case "pause":
