...
const { SlashCommandBuilder, InteractionContextType, ThreadAutoArchiveDuration, ChannelType, PermissionFlagsBits, EmbedBuilder, MessageFlags } = require("discord.js");
//const { createTranscript } = require("discord-html-transcripts");
const { createTranscript } = require("discord-html-transcripts-v2");
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .addStringOption(option =>
                    option
                        .setName("state")
                        .setDescription("Close ticket action")
                        .setRequired(true)
                        .setChoices(
                        {
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName("unlock")
                .setDescription("Unlocks the ticket thread.")
        )
        .addSubcommand(subcommand =>
            subcommand
                .setName("list")
    ...
    async execute(interaction) {
        ...
        const isTicketsEnabled = serverSettings.get("enableUserTickets");
        ...
        let ticketData, ticketState, ticketThread; // reserved variables for ticket command
        if (!isTicketsEnabled) return interaction.editReply({
            content: "User ticket system is disabled!"
        });
        ...
        if (!ticketsModRole) return interaction.editReply({
            content: "No ticket moderator or staff role has been configured! Please configure it using `/settings`."
        });
        async function updateTicketData(thread, state = options.getString("state")) {
            try {
        ...
        async function transcribeTicket(thread) {
            ...
            try {
                await archiveChannel.send({
                    //content: "archivedTicket.message_content",
                    //embeds: [],
                    files: [threadTranscript]
                });
            } catch (err) {
                logger.warn(`Failed to send ticket transcript to ${archiveChannel} in ${guild.name}!`);
                logger.error(`${err.name}: ${err.message}`);
                logger.debug(err.stack);
            };
            try {
                await ticketOwner.send({
                    content: "Your ticket has been closed. Here is your ticket transcript for personal records.",
                    files: [threadTranscript]
                });
            } catch (err) {
        ...
        if (subcommand === "close") {
            if (interaction.channel.isThread()) {
                ticketThread = ticketsChannel.threads.resolve(interaction.channel);
        ...
        if (subcommand === "lock" || subcommand === "unlock") {
            ...
            if (ticketThread.isThread()) {
                ...
                ticketState = ticketData.get("ticketState");
                if (member.permissions.has(PermissionFlagsBits.ManageThreads) && member.roles.cache.has(ticketsModRole.id)) {
                    if (subcommand === "lock") {
                        if (ticketThread.locked && ticketState === schema.tickets.states.LOCKED) return interaction.editReply({
                            content: "This ticket is already locked to moderators only!"
                        });
                        await updateTicketData(ticketThread, "LOCKED");
                        await ticketThread.setLocked(true);
                        await ticketThread.send({
                            content: `Locked by ${member} to moderators only.`
                        });
                    };
                    if (subcommand === "unlock") {
                        if (!ticketThread.locked && ticketState === schema.tickets.states.OPEN) return interaction.editReply({
                            content: "This ticket is already open to new messages!"
                        });
                        await updateTicketData(ticketThread, "OPEN");
                        await ticketThread.setLocked(false);
                        await ticketThread.send({
                            content: `Opened by ${member} to all members.`
                        });
                    };
                    await interaction.editReply({
