...
if (!config.storage.password || config.storage.password == "") logger.warn("No database password provided! Is this app's database setup without a password?");
// verify application environment values before starting
if (!process.env.DISCORD_TOKEN) {
    logger.error("Missing or undefined parameter \"DISCORD_TOKEN\"! Stopping bot due to missing authentication token.");
    process.exit(-1);
};
