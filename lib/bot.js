...
const { generateCrashReport } = require('./utils/handlers/crashReporter');
...
const client = new Client({
    ...
    presence: {
        activities: [
            { name: "custom_status.app_starting", state: "Starting up...", type: ActivityType.Custom }
        ],
        status: "dnd"
    },
    // bot custom properties (LEGACY)
    config: {
        developers: config?.developer.devUserIds ?? [],
        admins: config?.general.adminUserIds ?? []
    },
});
...
client.storage = require("./utils/storage");
// client application commands loader
    ...
    for (const file of cmdFiles) {
        ...
        try {
            ...
            if ("data" in command && "execute" in command) {
                ...
                logger.debug(`Loaded command.${cmdSubDir}.${command.data.name} from file "${command.path}"`);
            } else {
                const reason = () => {
