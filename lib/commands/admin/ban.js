...
const { SlashCommandBuilder, PermissionFlagsBits, InteractionContextType, MessageFlags, ActionRowBuilder, ButtonBuilder, ButtonStyle, userMention } = require("discord.js");
                ...
                if (res.customId === "ban_confirm") {
                    try {
                        if (member.bannable) {
                            ...
                            if (options.getBoolean("soft")) setTimeout(async () => await guild.members.unban(member.user.id), 1000);
                        } else {
