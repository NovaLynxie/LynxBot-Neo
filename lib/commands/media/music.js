...
module.exports = {
    ...
    async execute(interaction) {
        ...
        async function addSong({ title, duration, url, source }) {
            try {
                ...
                return interaction.editReply({
                    content: `Added "${title} to the music queue!`
                });
            } catch (err) {
                logger.error(`${err.name}: ${err.message}`);
                logger.debug(err.stack);
                return interaction.editReply({
                    content: `Failed to add "${title}" to the music queue!`
                });
            };
        };
        switch (subcmdgrp ?? subcommand) {
            case "play":
                if (!resource || resource.ended) {
                    ...
                    // TODO - Add music queue functionality!
                };
                if (!connection) {
