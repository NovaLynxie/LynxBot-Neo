...
const https = require("node:https"); // use https module for https streams
const schema = require("../../assets/schemas/music.json");
const { generateDependencyReport, getVoiceConnection, createAudioPlayer, NoSubscriberBehavior, createAudioResource, joinVoiceChannel, AudioPlayerStatus, VoiceConnectionStatus } = require("@discordjs/voice");
const { SlashCommandBuilder, InteractionContextType, EmbedBuilder, MessageFlags, bold, codeBlock } = require("discord.js");
...
module.exports = {
            ...
            subcommand
                ...
                .addBooleanOption(option =>
                    option
                        ...
                        .setDescription("Show dependency report?")
                )
        )
        .addSubcommand(subcommand => 
            subcommand
        ...
        .addSubcommandGroup(subcmdgrp => 
            subcmdgrp
                ...
                .addSubcommand(subcommand =>
                    subcommand
                        .setName("link")
                        .setDescription("Stream from internet radio")
    ...
    async execute(interaction) {
        ...
        const { client, channel, guild, member, options } = interaction;
        ...
        const idleTimeout = (seconds) => setTimeout(() => {
        ...
        async function fetchWebStream(url) {
        ...
        async function loadSong(bump = false) {
            ...
            if (!nextSong) {
                ...
                if (nextSong.get("source") == schema.types.FILE) {
                    let file = {
                        data: await fetchWebStream(nextSong.get("url")),
        ...
        function validateUrlInput(url) {
            try {
                url = new URL(url); console.log(url);
        ...
        switch (subcmdgrp ?? subcommand) {
            case "debug":
                const debugEmbed = new EmbedBuilder()
                    .setTitle("Music Player Debug Information")
                    .setFields(
                        { name: "Guild Name", value: guild.name, inline: true },
                        { name: "Guild ID", value: guild.id, inline: true },
                        { name: "Voice Channel", value: voiceChannel ? `${voiceChannel.name} (${voiceChannel.id})` : "Not connected", inline: true },
                        { name: "Connection State", value: connection ? connection.state.status : "No connection", inline: true },
                        { name: "Audio Player State", value: audioPlayer ? audioPlayer.state.status : "No audio player", inline: true },
                        { name: "Current Resource", value: resource ? (resource.metadata && resource.metadata.common.title ? resource.metadata.common.title : "Unknown Title") : "No resource loaded", inline: true }
                    );
                    if (options.getBoolean("dependencies")) {
                        debugEmbed.addFields(
                            { name: "Dependency Report", value: codeBlock("yaml", generateDependencyReport()) }
                        );
                    };
                ...
                break;
            case "nowplaying":
                // TODO: now playing implementation?
                await interaction.editReply({
                    content: "NOW PLAYING command coming soon!"
                });
                break;
            case "play":
                let data = { title: "", url: "" };
                ...
                if (subcommand === "file") {
                    ...
                    const fileStream = await fetchWebStream(attachment.url);
                    const metadata = await parseStream(fileStream);
                    resource = createAudioResource(attachment.url, {
                        metadata: { url: attachment.url, type: schema.types.FILE, ...metadata } ?? null, inlineVolume: true
                    });
                    data.title = metadata.common.title ?? attachment.title ?? attachment.name;
                ...
                if (subcommand === "link") {
                    ...
                    const urlInput = options.getString("url");
                    if (!validateUrlInput(urlInput)) return interaction.editReply({
                        content: "Only secure stream urls (i.e. https) is allowed. Please check your input and try again."
                    });
                    const linkStream = await fetchWebStream(urlInput);
                    const metadata = await parseStream(linkStream);
                    resource = createAudioResource(urlInput, {
                        metadata: { url: urlInput, type: schema.types.STREAM, ...metadata } ?? null,
                        inlineVolume: true
                    });
                ...
                logger.debug(`User ${member.user.tag} (${member.user.id}) requested to play a song in guild ${guild.name} (${guild.id})`);
                if (!member.voice) {
                    return interaction.editReply({
                        content: "You are not in a voice channel!"
                    });
                };
                let queueCount = await Music.count({
                    where: { guildId: member.guild.id ?? guild.id }
                });
                if (queueCount > 1) {
                  await interaction.editReply({
                  ...
                  return; // if there are already songs in the queue, return here
                };
                if (!connection) {
                    logger.debug("No existing voice connection found, attempting to join user's voice channel...");
                ...
                client.music.resources.set(guild.id, resource);
                stopped = false;
                if (resource.metadata && resource.metadata.common.title) {
                    await interaction.editReply({
                        content: `Playing ${bold(`${resource.metadata.common.title} by ${resource.metadata.common.artist ?? "Unknown"}`)}!`
                    });
                    return voiceChannel.send({
                        content: `Playing ${bold(`${resource.metadata.common.title} by ${resource.metadata.common.artist ?? "Unknown"}`)}! \nRequested by ${member}`
                    });
                    ...
                    await interaction.editReply({
                        content: "No metadata was found! Playing next song anyway."
                    });
                    return voiceChannel.send({ content: `Playing audio file uploaded by ${member}` });
                };
            case "pause":
