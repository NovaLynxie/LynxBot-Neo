...
const { SlashCommandBuilder, PermissionFlagsBits, MessageFlags, bold } = require("discord.js");
...
module.exports = {
    ...
    enabled: true, // whether to enable command internally
    permsLevel: permissions.commands.DEVELOPER, // ignored if restricted is false
    ...
    async execute(interaction) {
        ...
        if (command) {
            ...
            try {
                logger.debug(`Reload command called by ${interaction.user.tag} in guild ${interaction.guild?.name || "DMs"} for command ${cmdName}`);
                ...
                logger.debug(`Reloaded command ${cmdName} successfully!`);
