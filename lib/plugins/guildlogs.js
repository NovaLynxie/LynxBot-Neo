const { AuditLogEvent, ChannelType, EmbedBuilder, Events, channelMention, codeBlock, time, Embed, Guild, User, TextChannel } = require("discord.js");
const { stripIndents } = require("common-tags");
const logger = require("../utils/logger")("guildlogs");
// guild log plugin helper functions
/**
 * Resolves Event enum keys.
 * @param {AuditLogEvent | Events} enumType - type of event to resolve
 * @param {string | number} enumKey - event enum value to resolve
 */
...
/**
 * Resolves permission changes and returns diff code block with result.
 */
function generatePermissionsDiff(oldPermissions, newPermissions) {
    const oldPermArray = oldPermissions.toArray(); // convert old permissions to array
    const newPermArray = newPermissions.toArray(); // convert new permissions to array
    ...
    return codeBlock("diff",`${grantedPerms.map(perm => `+ ${perm}`).join("\n")}\n${revokedPerms.map(perm => `- ${perm}`).join("\n")}`); // return formatted permission diff string
};
/**
 * Resolves permission override changes and returns diff code block with result.
 */
function generatePermissionOverwritesDiff(oldPermOverwrites, newPermOverwrites) {
    const oldAllowArray = oldPermOverwrites.allow.toArray(); // convert "allow" old perm overwrites to array
    const oldDenyArray = oldPermOverwrites.deny.toArray(); // convert "deny" old perm overwrites to array
    const newAllowArray = newPermOverwrites.allow.toArray(); // convert "allow" new perm overwrites to array
    const newDenyArray = newPermOverwrites.deny.toArray(); // convert "deny" new perm overwrites to array
...
/**
 * Sends guild log embed to specified guild.
 * @param {Embed} guildLogEmbed
 * @param {Guild} guild
 */
async function sendGuildLogEmbed(guildLogEmbed, guild) {
    const client = guild.client; // fetch client instance from guild object.
    // first fetch our guild log channels from "Settings" table in database
    const guildLogChannels = await client.storage.models.Settings.findOne({
        attributes: ["defaultChannel", "moderationChannel"],
        where: { guildId: guild.id },
    });
    if (!guildLogChannels || (!guildLogChannels?.defaultChannel && !guildLogChannels?.moderationChannel)) return null; // abort here if values return null or undefined. i.e. if settings are not set :)
    // resolve our channels using stored channel ids if possible
    const defaultLogsChannel = client.channels.resolve(guildLogChannels.defaultChannel);
    const moderationLogsChannel = client.channels.resolve(guildLogChannels.moderationChannel);
    if (!defaultLogsChannel && !moderationLogsChannel) return logger.debug(`No usable channels were found for server "${guild.name}". Are they missing or invalid?`); // abort sending if unresolved
    try {
        if (moderationLogsChannel) {
            await moderationLogsChannel.send({
                embeds: [guildLogEmbed],
            });
            return true;
        } else {
            await defaultLogsChannel.send({
                embeds: [guildLogEmbed],
            });
            return true;
        };
...
// guild log plugin main functions
/**
 * 
...
async function createAuditEventLog(action, guild, channel, executor, target) {
    // TODO: Implement auditLog embed output...? (currently unused)
    if (!guild)
        return logger.error(
            `AuditEventLog Error: ${Events.GuildAuditLogEntryCreate} called with missing "guild" parameter!`,
        );
    ...
    switch (action) {
        ...
        default:
            logger.debug(
                `AuditLogEvent action "${resolveEventEnumKey(AuditLogEvent, action)}" received, this is either unknown or not currently tracked! If you think this is an error, please contact my developer.`,
            );
    }
    auditEventLogEmbed.setTimestamp(); // adds current date timestamp before sending completed embed
    sendGuildLogEmbed(auditEventLogEmbed, guild); // pass embed instance and guild object to send function
};
/**
 * @param {string} event
 * @param {Guild} guild
...
async function createGuildEventLog(event, guild, ...args) {
    ...
    let auditLogs, channel, member, invite, role; // common shared variables used by all log events.
    try {
        auditLogs = await guild.fetchAuditLogs();
        auditLogs.firstEntry = auditLogs.entries.first();
    } catch (err) {
        logger.error(err.message);
        logger.debug(err.stack);
    };
    switch (event) {
        ...
        case Events.ChannelUpdate:
            ...
            let channelOverridesDiff = generateOverridePermissionDiff(oldChannel, newChannel);
    ...
    sendGuildLogEmbed(guildLogEmbed, guild); // pass embed instance and guild object to send function
};
