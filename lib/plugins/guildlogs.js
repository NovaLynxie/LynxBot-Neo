...
async function sendGuildLogEmbed(guildLogEmbed, guild) {
    ...
    if (!guildLogChannels || (!guildLogChannels?.defaultChannel && !guildLogChannels?.moderationChannel)) return; // abort here if values return null or undefined. i.e. if settings are not set :)
    ...
    try {
        if (moderationLogsChannel) {
            moderationLogsChannel.send({
                embeds: [guildLogEmbed],
            });
        } else {
            defaultLogsChannel.send({
                embeds: [guildLogEmbed],
            });
        };
    } catch (err) {
