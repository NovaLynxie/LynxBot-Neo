...
// print version information to application
logger.info(`${appName} v${appVersion} (discord.js v${discordJsVersion}) running NodeJS v${process.versions.node}`);
...
try {
    if (!existsSync("./config.toml")) {
        ...
        let configTemplate = readFileSync("./lib/assets/templates/default_config.toml", "utf-8");
