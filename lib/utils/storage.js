...
const models = {}; // all loaded models are added here to be referenced later

for (let module of readdirSync(dbModelsDir).filter((file) => file.endsWith(".model.js"))) {
    try {
        const model = require(`${dbModelsDir}/${module}`);
        if (["name", "category", "define"].every(key => key in model)) {
            const { name, category, define } = model;
            if (!models[category]) models[category] = {}; // define category property if not already added
            if (model.enabled === null ?? model.enabled === undefined) logger.warn(`Data model "${model.name}" is missing ENABLED flag! It will be skipped during loading.`);
            if (model.enabled === true) { // only load modules if enabled flag is set to TRUE, else it will be excluded during loading sequence
                models[category][name] = define.call(model, sequelize, DataTypes);
...
logger.debug(`Loaded ${Object.keys(models).length} data models.`);
logger.verbose(`Models => ${JSON.stringify(Object.keys(models))}`);
...
async function sync(alter =  false, force = false) {
    ...
    if (alter === true || force === true) {
        ...
        try {
            const backupDate = getFormattedDate(); // fetch backup date to append to file name
            logger.debug("Creating backup of database file...");
            await copyFile("./data/storage.db", `./data/storage-${backupDate}.db`);
            logger.info(`Database backup created! Saved to "./data/backups/app-storage-${backupDate}.db"`);
        } catch (err) {
            logger.error(`${err.name}: ${err.message}`);
            logger.debug(err.stack);
