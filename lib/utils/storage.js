...
const sequelize = new Sequelize("lynxbot_neo", config.storage.username, config.storage.password, {
    ...
    storage: "data/app_storage.db" // applies to "sqlite" storage only!
});
    ...
    switch (action) {
        case "guild":
            ...
            try {
                ...
                logger.error(`Failed to initialize database tables for "${guild.name}" (${guild.id})!`);
                logger.error(`${err.name}: ${err.message}`); logger.debug(err.stack);
            };
        case "user":
            // TODO - Implement user initialization function callback
            break;
        default:
...
async function sync(alter =  false, force = false) {
    ...
    logger.info("Synchronising database models...");
    if (alter === true || force === true) {
        ...
        try {
            ...
            await copyFile("./data/app_storage.db", `./data/app_storage-${getFormattedDate()}.db`);
            logger.info(`Database backup created! Saved to "./data/backups/app-storage-${getFormattedDate()}.db"`);
        } catch (err) {
            logger.error(`${err.name}: ${err.message}`);
            logger.debug(err.stack);
            logger.warn("Aborted backup action to prevent data loss!");
